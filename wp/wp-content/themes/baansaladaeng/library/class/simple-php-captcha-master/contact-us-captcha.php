...
//
function contact_us_captcha($config = array()) {
...
//		'characters' => 'ABCDEFGHJKLMNPRSTUVWXYZabcdefghjkmnprstuvwxyz23456789',
	...
	$image_src = substr(__FILE__, strlen( realpath($_SERVER['DOCUMENT_ROOT']) )) . '?_captcha_contact_us_conf&amp;t=' . urlencode(microtime());
	$image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
	
	$_SESSION['_captcha_contact_us_conf']['config'] = serialize($captcha_config);
...
// Draw the image
if( isset($_GET['_captcha_contact_us_conf']) ) {
	
	session_start();
	
	$captcha_config = unserialize($_SESSION['_captcha_contact_us_conf']['config']);
	if( !$captcha_config ) exit();
	
	unset($_SESSION['_captcha_contact_us_conf']);
